...
LOG_FILE="$PROJECT_ROOT/serial_outputs.txt"
...
echo "[SETUP] Building serial app with make serial..." | tee -a "$LOG_FILE"
make serial 2>&1 | tee -a "$LOG_FILE"
...
for iteration in 1 2 3 4 5; do
	{
		...
		echo "Command: ./serial_app $INPUT_DIR"
		echo "------------------------------------------------------------"
	} | tee -a "$LOG_FILE"

	./serial_app "$INPUT_DIR" 2>&1 | tee -a "$LOG_FILE"
